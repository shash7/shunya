 ...
 * ---------------------------------------------------------------- */
	...
	var resources = [
		...
		},
		{
			...
			template : 'options',
			color : '#DAFCF5',
			description : 'Modify global fields here'
		},
		{
			type : 'page',
			name : 'Settings',
			...
			navName : 'settings',
			template : 'settings',
			description : 'Set global settings here'
		}
	];
	...
	var cms = {
		...
		defineType : function(obj) {
			if(obj.schema && obj.name) {
				db.createType(obj.schema, obj.name, obj);
 ...
 * ---------------------------------------------------------------- */
	...
	function initializeRoutes() {
		 ...
		 *
		 * Now we'll proceed to setup routes:-
		 */
		...
		// Backend routes
		app.use(app.locals.paths.backend, backend(app, resources));
