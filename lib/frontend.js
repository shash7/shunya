...
(function() {
	...
	function checkAjax(req, res, next) {
		...
		if(contentType === 'application/json' || contentType.indexOf('application/json') === 0) {
			...
			if(method === 'GET') {
				ajaxHandler.get.typePage(req, res, next);
			} else if(method === 'POST') {
				console.log()
				res.send('post');
			} else { // 404
	...
	var ajaxHandler = {
			...
			typePage : function(req, res, next) {
				...
				db[type].findById({_id : slug}, function(err, page) {
					if(page) {
						...
						delete res.locals.paths; // Don't need paths, possible server security compromise
						res.type('json').status(200).json(res.locals);
					} else {
						res.send(404);
					}
		...
		post : {
			type : function(req, res) {
				var data     = req.body;
				var typeName = res.locals.type.name;
				var obj = {};
				if(data.fields && typeof data.fields === 'object') {
					// Check if array or object. If array, bulk create posts
					if(data.fields.constructor === Array) { // Bulk post
						res.send('Not supported yet');
					} else { // Single post
						data = data.fields;
						data.dateCreated = data.dateCreated || '';
						data.dateCreated = new Date();
						obj =  new db[typeName](data);
						obj.save(function() {
							res.type('json').status(201).json(obj);
						});
					}
				} else {
					res.send(400);
 ...
 * ---------------------------------------------------------------- */
	module.exports = function(app) {
		...
		router.route('/:type')
			...
			.post(ajaxHandler.post.type); // Create resource
